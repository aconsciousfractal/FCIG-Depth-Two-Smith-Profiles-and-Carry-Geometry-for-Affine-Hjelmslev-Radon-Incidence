...
\cref{sec:S09} are bounded computational cross-checks only.  The theorem
includes $p=5$.  The companion does not execute the memory-intensive
full-matrix check at $(p,n)=(5,2)$; this computational resource boundary is
not a mathematical exception and is not used in the proof.  No unexecuted
experimental route is used.
...
mutations, and evaluates the shipped full-size companion constructions.  It
is bounded computational cross-checking, not a substitute for any
all-parameter proof.  The uniform theorem includes $p=5$, while the
memory-intensive full-matrix companion check at $(p,n)=(5,2)$ is deliberately
not executed or reported.  No unexecuted generic-orbit experiment enters
this manuscript.
...
rejects invalid primes, strict-sublattice mutations, and the reserved
full-matrix target\\
...
Under the author's direction, OpenAI Codex, using the GPT-5.6 Sol model,
generated the central mathematical development, including the all-prime
depth-two Smith profile and the transfer-and-carry analysis, and assisted
with the exact companion software and manuscript.  The author selected the
research direction, challenged and checked the derivations and outputs, set
the public boundaries, and takes responsibility for the final content.
Same-workflow machine reviews are not human peer review or independent expert
verification; the repository's \path{AI_USE.md} records the detailed
provenance.
